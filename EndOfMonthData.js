(function(soql) {
    ...
    Q.allSettled([new soql.multipart(accountQuery), new soql.multipart(salesQuery), new soql.multipart(forecastQuery), new soql.multipart(dateQuery)]).spread(function (resAccount, resSales, resForecast, resDate) {
        ...
        _.each(result, function(acc) {
            ...
            acc.sales = d3.sum(sales, function(s) { console.log(s); return s.Account__r.Id == acc.Id && s.Fiscal_Year__c == 2015 && s.Is_Fiscal_Year_To_Date__c == true ? s.Net_Value__c : 0; })
            ...
            acc.budget = d3.sum(forecast, function(s) { console.log(s); return s.Account__r.Id == acc.Id && s.Fiscal_Year__c == 2015 && s.Is_Fiscal_Year_To_Date__c == true && s.Forecast_Type__c == 'Budget' ? s.Net_Value__c : 0; })
